...
fn run_program(prog: &mut Vec<isize>) -> isize {
    let mut instr_ptr = 0;
...
fn parse_str(prog_str: &str) -> Vec<isize> {
    prog_str.split(',')
...
mod test {
    ...
    #[test]
    fn test_computer_runs() {
        let res = run_program(&mut parse_str(TEST_STR));
        assert_eq!(pt1(TEST_STR), TEST_OUT);
    }
